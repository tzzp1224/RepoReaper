...
**RepoReaper** 超越了传统的“代码对话”工具，它是一个模拟高级技术专家认知过程的**自治型代码审计 Agent**。

传统的 RAG（检索增强生成）往往依赖静态的文本切片，容易丢失代码逻辑。RepoReaper 创造性地引入了 **AST（抽象语法树）解析** 和 **ReAct 推理循环**。它不仅仅是检索，更是在“收割”知识——Agent 会像人类架构师一样，先建立全局地图，再根据问题自主决定去读取哪些文件，实时填补知识盲区。
